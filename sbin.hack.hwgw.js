...
export async function main(ns) {
  ...
  //ns.tprint(`servers=${servers.length}`)
  let available_ram = new Map();
  //this.admin && (this.level <= this.ns.getPlayer().skills.hacking))}
  //ns.tprint(`purchased=${servers[0].purchased}, isHome=${servers[0].isHome}, money.max=${servers[0].money.max}, hacking=${ns.getPlayer().skills.hacking}`)
  //ns.tprint(`admin=${servers[0].admin}, level=${servers[0].level}, ports.open=${servers[0].ports.open}, ports.required=${servers[0].ports.required}`)
  ...
  while (true) {
    ...
    ns.print(`ready_targets=${ready_targets.length}, unready_targets=${unready_targets.length}, required_ready_targets=${required_ready_targets}`)
    if (ready_targets.length >= required_ready_targets) {
      ...
      for (let target of unready_targets) {
        ...
        ns.print(`proposed_batch ready_targets=${ready_targets.length}, unready_targets=${unready_targets.length}, required_ready_targets=${required_ready_targets}`)
        let result = run_hwgw_batch(ns, attackers, target, proposed_batch, available_ram);
        if (result) {
          available_ram = result;
 ...
 * @param {HackableBaseServer} target 
 * @param {Object} batch 
 * @param {Map<id,ram>} available_ram 
 * @returns 
 */
function run_hwgw_batch(ns, attackers, target, batch, available_ram) {
  ...
  ns.print(`batch: grScheduled:${grScheduled.reduce((a, b) => a + b.threads, 0)}, growThreads:${growThreads}`)
  //ns.print(`batch: weakThreads1:${weakThreads1}, weakThreads2:${weakThreads2}, hackThreads:${hackThreads}, growThreads:${growThreads}`)

  // make sure our batch matches the threads requested initially
  // exec and return modified ram
  ns.print(`batch: wkSanityCheck:${wkSanityCheck}, hkSanityCheck:${hkSanityCheck}, grSanityCheck:${grSanityCheck}`)
  if (wkSanityCheck && hkSanityCheck && grSanityCheck) {
    for (let cmd of nextBatch) {
      ns.print(`Running ${cmd.filename} on ${cmd.attacker}:${cmd.threads} to attack ${target.id}: ${cmd.landing}`)
